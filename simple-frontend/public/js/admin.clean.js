// Admin Dashboard JavaScript (cleaned & updated for Lucide icons)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // replace Lucide icons present in static HTML
    if (window.lucide) {
        try { lucide.replace({ 'stroke-width': 1.5 }); } catch (e) { /* ignore */ }
    }
});
            ...
            <div class="activity-item">
                ...
                <div class="activity-content">
                    ...
                    <div class="activity-meta">
                        To: ${escapeHtml(activity.userId)} • 
                        ${escapeHtml((activity.channels || []).join(', ')) || 'Unknown'} • 
                        ${formatTime(activity.sentAt)}
                        ${activity.reaction ? ` • ${activity.reaction === 'like' ? '<i data-lucide="thumbs-up"></i>' : '<i data-lucide="thumbs-down"></i>'}` : ''}
                    </div>
                </div>
                <span class="activity-status ${activity.status?.toLowerCase() || ''}">${escapeHtml(activity.status)}</span>
            </div>
        `).join('');
...
function renderUsersTable(users) {
    ...
    tbody.innerHTML = users.map(user => `
        <tr>
            ...
            <td>
                ${user.hasTelegram ? 
                    `<span class="telegram-badge"><i data-lucide="smartphone"></i> ${escapeHtml(user.telegramUsername || 'Connected')}</span>` : 
                    '-'}
            </td>
            <td>
            ...
            <td>${escapeHtml(user.frequencyLabel || '-')}</td>
...
async function triggerScraping(btn) {
    ...
    try {
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = '<i data-lucide="clock"></i> Scraping...';
            if (window.lucide) try { lucide.replace(); } catch (e) {}
        }
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = originalHtml || '<i data-lucide="play"></i> Trigger Scraping';
            if (window.lucide) try { lucide.replace(); } catch (e) {}
        }
    }
}
...
function updateStatusChart(data) {
    ...
    statusChart = new Chart(ctx, {
        type: 'bar',
        data: { labels: labels, datasets: [{ data: values, backgroundColor: labels.map(l => colors[l] || '#6366f1') }] },
        options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    });
}
...
function getActivityIcon(status) {
    switch (status) {
        case 'SENT': return '<i data-lucide="check-circle"></i>';
        case 'PENDING': return '<i data-lucide="clock"></i>';
        case 'FAILED': return '<i data-lucide="x-circle"></i>';
        default: return '<i data-lucide="mail"></i>';
    }
}
        ...
        <label class="user-checkbox">
            ...
            <span class="user-info">
                ...
                <span class="user-channels">
                    ${user.email ? '<i data-lucide="mail"></i>' : ''}
                    ${user.hasTelegram ? '<i data-lucide="smartphone"></i>' : ''}
                </span>
            </span>
        </label>
    `).join('');
...
async function sendBroadcast(btn) {
    ...
    const originalHtml = btn ? btn.innerHTML : '<i data-lucide="send"></i> Send Broadcast';
    ...
    try {
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = '<i data-lucide="clock"></i> Sending...';
            if (window.lucide) try { lucide.replace(); } catch (e) {}
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (subjectInput) subjectInput.addEventListener('input', updateBroadcastPreview);
    if (messageInput) messageInput.addEventListener('input', updateBroadcastPreview);
});
